...
public:
    ...
    void firstLife() {
        for(int i=20;i<60;i++)
        map[i] = {GEN_FOTO | GEN_MOVE | GEN_FINDPARTNER, STATUS_MALE, GSTATUS_ENABLE, 0, 0, 0};
        for(int i=80;i<120;i++)
        map[i] = {GEN_FOTO | GEN_MOVE | GEN_FINDPARTNER, STATUS_FEMALE, GSTATUS_ENABLE, 0, 0, 0};
    }
};
...
public:
    ...
    void run(Zoo& zoo, int pos) {
        ...
        if (((zoo.flags & GEN_MOVE) != 0) && zoo.power >= 5) {
            ...
            int ran = die() % 4;
            switch (ran) {
                case 1: if (world.isAllowUp(pos)) {
                        world.MoveTo(world.map[pos], world.map[pos - world.map_size]);
                        zoo.power -= 2;
                        return;
                    }
                case 2: if (world.isAllowRight(pos)) {
                        world.MoveTo(world.map[pos], world.map[pos + 1]);
                        zoo.power -= 2;
                        return;
                    }
                case 3: if (world.isAllowDown(pos)) {
                        world.MoveTo(world.map[pos], world.map[pos + world.map_size]);
                        zoo.power -= 2;
                        return;
                    }
                case 4: if (world.isAllowLeft(pos)) {
                        world.MoveTo(world.map[pos], world.map[pos - 1]);
                        zoo.power -= 2;
                        return;
                    }
        ...
        zoo.age++;
        if (!world.isAllowUp(pos) && !world.isAllowRight(pos) && !world.isAllowLeft(pos) && !world.isAllowDown(pos)) zoo.power -= 4;
        if (zoo.power < 0) {
    ...
    void cycle() {
        for (int i = 0; i < world.map_maxpos; ++i) {
            if ((world.map[i].gstatus & GSTATUS_ENABLE) != 0) run(world.map[i], i);
        }
    }
};
